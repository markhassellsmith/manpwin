    ...
    for (int i = 1; i <= config.maxIterations; i++)
    {
        ...
        if (config.detectCycles)
        {
            if (visitedPoints.find(std::make_pair(x, y)) != visitedPoints.end())
            {
                ...
                // Log cycle detection
                char msg[256];
                sprintf(msg, "CYCLE DETECTED! Point (%d, %d) repeats at step %d, first seen at step %d. Cycle length: %d",
...
**************************************************************************/
void CHailstone::DrawSequence(CPlot& plot, double centerX, double centerY,
                              float pixelsPerUnitX, float pixelsPerUnitY, 
                              int screenWidth, int screenHeight, long threshold)
{
    if (m_points.size() < 2)
