...
void	COtherFunctions::set_Plasma_palette(int rotate(int direction))
    ...
    TrueCol->PalettePtr[0] = {0,0,0};
    for (i = 1; i <= 85; i++)
	{
	TrueCol->PalettePtr[i].rgbtRed = (i*Green.rgbtRed + (86 - i)*Blue.rgbtRed) / 85;
	TrueCol->PalettePtr[i].rgbtGreen = (i*Green.rgbtGreen + (86 - i)*Blue.rgbtGreen) / 85;
	TrueCol->PalettePtr[i].rgbtBlue = (i*Green.rgbtBlue + (86 - i)*Blue.rgbtBlue) / 85;

	TrueCol->PalettePtr[i + 255].rgbtRed = (i*Red.rgbtRed + (86 - i)*Green.rgbtRed) / 85;
	TrueCol->PalettePtr[i + 255].rgbtGreen = (i*Red.rgbtGreen + (86 - i)*Green.rgbtGreen) / 85;
	TrueCol->PalettePtr[i + 255].rgbtBlue = (i*Red.rgbtBlue + (86 - i)*Green.rgbtBlue) / 85;

	TrueCol->PalettePtr[i + 510].rgbtRed = (i*Blue.rgbtRed + (86 - i)*Red.rgbtRed) / 85;
	TrueCol->PalettePtr[i + 510].rgbtGreen = (i*Blue.rgbtGreen + (86 - i)*Red.rgbtGreen) / 85;
	TrueCol->PalettePtr[i + 510].rgbtBlue = (i*Blue.rgbtBlue + (86 - i)*Red.rgbtBlue) / 85;
	}
    rotate(1);
    }
